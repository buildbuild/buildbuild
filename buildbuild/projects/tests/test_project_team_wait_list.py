...
from projects.models import Project, ProjectTeamWaitList 
...
class project_manager_test(TestCase):
    def setUp(self):
        ...
        self.project_team_wait_list = ProjectTeamWaitList.objects.create(
            project = self.project,
            wait_team = self.team
        )
        self.project_team_wait_list.save()
    ...
    def test_project_team_wait_list_must_have_date_joined(self):
        try:
            self.project_team_wait_list.date_requested
        except AttributeError:
            self.fail("project_team_wait_list doesn't have date_requested")
